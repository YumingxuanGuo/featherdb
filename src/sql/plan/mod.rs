...
impl Plan {
    ...
    /// Optimizes the plan, consuming it and returning a new plan.
...
impl Node {
    /// Recursively transforms nodes by applying functions before and after descending. TODO: Read.
    ...
    /// Transforms all expressions in a node by calling .transform() on them with the given closure. TODO: Read.
        ...
        A: Fn(Expression) -> Result<Expression>,
    {
        Ok(match self {
            ...
            Self::Insert { table, columns, expression } => Self::Insert {
                table,
                columns,
    ...
    pub fn format(&self, mut indent: String, root: bool, last: bool) -> String {
        ...
        match self {
            ...
            //     s += "\n";
            // }
            Self::Insert { table, columns: _, expression } => {
                s += &format!("Insert: {} ({} rows)\n", table, expression.len());
            }
