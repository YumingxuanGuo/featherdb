...
async fn test_basic_replication() -> Result<()> {
    ...
    cluster.nodes[leader as usize].node.start(Command { session_id: 0, operation: b"abc".to_vec() })?;
    cluster.nodes[leader as usize].node.start(Command { session_id: 0, operation: b"def".to_vec() })?;
    cluster.nodes[leader as usize].node.start(Command { session_id: 0, operation: b"ghi".to_vec() })?;
    for _ in 0..3 {
        for i in 0..3 {
