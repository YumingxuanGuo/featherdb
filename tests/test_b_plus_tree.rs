...
// use gman_db::storage::index::StoreDisk;
// use gman_db::storage::index::b_plus_tree::{BPlusTree, BPT};
...
// use gman_db::buffer::buffer_pool_manager::{BufferPoolManager, BPM};
...
//     let disk_manager = DiskManager::new();
//     let bpm = BPM::new(buffer_pool_size, disk_manager, k);

//     let mut tree = BPT::new(String::from("my_tree"), bpm, 3, 3);
...
//     tree.insert(13, RID { page_id: 13, slot_num: 13 }).unwrap();
//     tree.insert(11, RID { page_id: 11, slot_num: 11 }).unwrap();
//     tree.insert(15, RID { page_id: 15, slot_num: 15 }).unwrap();
//     tree.insert(8, RID { page_id: 8, slot_num: 8 }).unwrap();
//     tree.insert(14, RID { page_id: 14, slot_num: 14 }).unwrap();
//     tree.insert(9, RID { page_id: 9, slot_num: 9 }).unwrap();
//     tree.insert(12, RID { page_id: 12, slot_num: 12 }).unwrap();
//     tree.insert(10, RID { page_id: 10, slot_num: 10 }).unwrap();
...
// #[test]
// fn test_launch_parallel() {
//     let buffer_pool_size = 9;
//     let k = 10;
  
//     let disk_manager = DiskManager::new();
//     let bpm = BPM::new(buffer_pool_size, disk_manager, k);

//     let tree = BPT::new(String::from("my_tree"), bpm, 100, 100);
...
//                 guard.insert(j*10+count, RID { page_id: j*10+count, slot_num: (j*10+count) as u32 }).unwrap();
//             }
//             for j in 0..10 {
//                 let guard = t.read().unwrap();
//                 let val = guard.get_value(&(j*10+count));
//                 assert!(val.is_some());
//                 assert_eq!(val.unwrap().page_id, j*10+count);
//                 assert_eq!(val.unwrap().slot_num, (j*10+count) as u32);
//             }
//         });
//         handles.push(handle);
//     }
